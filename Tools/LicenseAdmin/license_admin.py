...
def initialize(private=PRIVATE,public=ROOT/"App/license-public-key.json"):
    ...
    keypath=private/"signing-key.dpapi"
    public=Path(public)
    if keypath.exists() or public.exists():
        raise ValueError("Key already exists. Refusing to overwrite.")
    key=Ed25519PrivateKey.generate()
    encrypted=transform(key.private_bytes_raw(),True)
    with keypath.open("xb") as f:
        f.write(encrypted)
...
def issue(request,expires=BETA_EXPIRES,private=PRIVATE,now=None):
    ...
    if expiry<=now or expiry>timestamp(BETA_EXPIRES):
        raise ValueError("Expiry must be in the future, no later than the beta deadline.")

    hashed=hashlib.sha256(req["serial"].encode("ascii")).hexdigest()

    if req["serial"].startswith("VX-DEV-"):
        dev_allowed=json.loads((private/"dev-serial-hash.json").read_text(encoding="ascii"))
        if not isinstance(dev_allowed,str) or hashed!=dev_allowed:
            raise ValueError("Serial not in the developer license.")
    else:
        allowed=json.loads((private/"serial-hashes.json").read_text(encoding="ascii"))
        if len(allowed)!=10 or len(set(allowed))!=10 or hashed not in allowed:
            raise ValueError("Serial not in the ten beta licenses.")
    ...
    with closing(sqlite3.connect(private/"issuance.sqlite",timeout=10)) as db, db:
        ...
        if old:
            if old[0]!=req["device"]:
                raise ValueError("This serial is already bound to another device. Offline tokens cannot be remotely revoked.")
            return old[1]

        key=Ed25519PrivateKey.from_private_bytes(
            transform((private/"signing-key.dpapi").read_bytes(),False)
        )
        ...
        token="VXLIC1."+encode(payload)+"."+encode(key.sign(payload))
...
def main():
    ...
    parser.add_argument("--expires",default=BETA_EXPIRES)
    args=parser.parse_args()

    try:
        if args.action=="init":
            initialize()
            print("Signing key created in Private/LicenseAdmin; DO NOT DISTRIBUTE.")
            return 0

        if not args.request_file or not args.output:
            parser.error("issue requires --request-file and --output")

        if args.output.exists():
            raise ValueError("Output already exists; refusing to overwrite.")

        token=issue(
            args.request_file.read_text(encoding="utf-8-sig").strip(),
            args.expires
        )

        args.output.write_text(token,encoding="ascii")
        print("Activation token saved. Serial masked in console.")
        return 0

    except Exception as e:
        print("License operation failed: "+str(e),file=sys.stderr)
        return 1
